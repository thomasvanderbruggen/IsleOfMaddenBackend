...
app.get('/api/player/:rosterId', (req, res) => { 
    ...
    con.query(sql, (err, sqlRes) => { 
        ...
        if (position === 'qb' || position === 'QB') {
            ...
            from passing_stats p left join rushing_stats r ON p.rosterId = r.rosterId and p.weekIndex = r.weekIndex and p.scheduleId = r.scheduleId where p.rosterId = ${rosterId} and p.seasonIndex = ${year} order by (p.weekIndex) asc;`;
            let seasonStats = {
                "name": '', 
                "rushAttempts": 0, 
                "rushBTackles": 0, 
            ...
            re.recToPct, re.recYds, re.recYdsAfterCatch, re.recYdsPerGame from rushing_stats ru left join receiving_stats re ON ru.rosterId = re.rosterId and ru.weekIndex = re.weekIndex  and ru.scheduleId = re.scheduleId where ru.rosterId = ${rosterId} and re.seasonIndex = ${year}`; 
           ...
           let seasonStats = { 
            ...
            sql = SQL`select recCatches, recCatchPct, recDrops, recLongest, recPts, recTDs, recYdsAfterCatch, recYacPerCatch, recYds, recYdsPerCatch, recYdsPerGame, fullName from receiving_stats where rosterId = ${rosterId} and seasonIndex = ${year}`;
            con.query(sql, (err, sqlRes) => { {
                if (err) {res.sendStatus(500);}
                else{
                    let weeklyStats = [];  
        ...
        position === 'cb' || position === 'def'){
            ...
            sql = SQL`select defCatchAllowed, defDeflections, defForcedFum, defFumRec, defInts, defIntReturnYds, defPts, defSacks, defSafeties, defTDs, defTotalTackles, fullName from defensive_stats where seasonIndex = ${year} and rosterId = ${rosterId}`; 
            con.query(sql, (err, sqlRes) => { 
                if (err) res.sendStatus(500);
                else { 
                    let weeklyStats = []; 
            ...
            sql = SQL`select p.puntsBlocked, p.puntsIn20, p.puntLongest, p.puntTBs, p.puntNetYds, p.puntAtt, p.puntYds, p.fullName, k.kickoffAtt, k.kickoffTBs from punting_stats p left join kicking_stats k ON p.rosterId = k.rosterId where p.seasonIndex = ${year} and p.rosterId = ${rosterId}`; 
            con.query(sql, (err, sqlRes) => { 
                if (err) {
                    res.sendStatus(500);
                    console.log(err); 
            ...
            sql = SQL`select kickPts, fGAtt, fG50PlusAtt, fG50PlusMade, fGLongest, fGMade, kickoffAtt, kickoffTBs, xPAtt, xPMade, xPCompPct, fullName from kicking_stats where seasonIndex = ${year} and rosterId = ${rosterId}`;
            con.query(sql, (err, sqlRes) => { 
                if (err) res.sendStatus(500); 
                else {
                    let weeklyStats = [];  
