...
app.get('/api/allPlayers', (req, res) => {
    if (firstRun) { 
        ...
        let sql = SQL`select firstName, lastName, devTrait, age, height, weight, playerBestOvr, speedRating, awareRating from players order by concat(firstName, lastName) desc;`; 
        con.query(sql, (err, sqlRes) => { 
            if (err) res.sendStatus(500); 
            else { 
                res.send(sqlRes);
