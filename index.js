...
app.get('/api/seasonstats/:year/:position/:playerId', (req, res) => { 
    ...
    if (position === 'qb' || position === 'QB') {
        ...
        from passing_stats p left join rushing_stats r ON p.rosterId = r.rosterId and p.weekIndex = r.weekIndex where p.rosterId = ${playerId} and p.seasonIndex = ${year} order by (p.weekIndex) asc;`;
        let seasonStats = {
            "name": '', 
            "rushAttempts": 0, 
            "rushBTackles": 0, 
