...
const gameStats = (gameId, res) => {
    ...
    sql = "select re.recCatches, re.recLongest, re.recYds, re.recTDs, re.fullName, re.teamId, ru.rushFum from receiving_stats re left join rushing_stats ru on re.playerId = ru.playerId and re.scheduleId = ru.scheduleId where re.scheduleId = ? and re.seasonIndex = 2 and (re.recCatches > 3 or re.recYds > 30 or re.recTDs >= 1)"; 
    con.query(sql, [gameId], (err, sqlRes) => {
        if (err) {
            sent = true;
            res.sendStatus(500); 
 ...
 const playerInfo = (playerId, res) => {
    ...
    con.query(sql, [playerId], (err, sqlRes) => { 
        ...
        if (position === 'qb' || position === 'QB') {
            ...
            FROM passing_stats p LEFT JOIN rushing_stats r ON p.playerId = r.playerId AND p.weekIndex = r.weekIndex AND p.scheduleId = r.scheduleId 
            LEFT JOIN players pl ON pl.playerId = p.playerId LEFT JOIN schedules sch ON sch.scheduleId = p.scheduleId WHERE p.playerId = ${playerId} AND p.seasonIndex = 2 AND p.weekIndex < 24 ORDER BY (p.weekIndex) ASC;`;
            let seasonStats = {
                "name": '', 
                "rushAttempts": 0, 
                "rushBTackles": 0, 
            ...
            re.recToPct, re.recYds, re.recYdsAfterCatch, re.recYdsPerGame, re.weekIndex, ru.teamId, sch.homeTeamId, sch.awayTeamId from rushing_stats ru left join receiving_stats re ON ru.playerId = re.playerId and ru.weekIndex = re.weekIndex  and ru.scheduleId = re.scheduleId left join players pl ON pl.playerId = ru.playerId left join schedules sch on sch.scheduleId = ru.scheduleId where ru.playerId = ${playerId} and re.seasonIndex = 2 and re.weekIndex < 24 order by (ru.weekIndex) asc`; 
        ...
        position === 'cb' || position === 'def'){
            ...
            sql = SQL`select p.puntsBlocked, p.puntsIn20, p.puntLongest, p.puntTBs, p.puntNetYds, p.puntAtt, p.puntYds, p.fullName, k.kickoffAtt, k.kickoffTBs, p.weekIndex, k.teamId, sch.awayTeamId, sch.homeTeamId from punting_stats p left join kicking_stats k ON p.playerId = k.playerId and p.scheduleId = k.scheduleId and p.weekIndex = k.weekIndex left join players pl on pl.playerId = p.playerId left join schedules sch on sch.scheduleId = p.scheduleId where p.seasonIndex = 2 and p.playerId = ${playerId} and p.weekIndex < 24 order by (p.weekIndex) asc`; 
            con.query(sql, (err, secondQuery) => { 
                if (err) {
                    res.sendStatus(500);
                }
...
*/
let sql = 'select fullName, playerId, sum(passAtt) "passAtt", sum(passComp) "passComp", sum(passYds) "passYds", sum(passTDs) "passTDs" from passing_stats where seasonIndex = 2 and weekIndex < 24 group by playerId order by sum(passYds) desc, sum(passTDs) desc LIMIT 10; select fullName, playerId, sum(rushAtt) "rushAtt", sum(rushYds) "rushYds", sum(rushTDs) "rushTDs", max(rushLongest) "rushLongest" from rushing_stats where seasonIndex = 2 and weekIndex < 24 group by playerId order by sum(rushYds) desc, sum(rushTDs) desc LIMIT 10; select fullName, playerId, sum(recCatches) "recCatches", sum(recYds) "recYds", sum(recTDs) "recTDs", max(recLongest) "recLongest" from receiving_stats where seasonIndex = 2 and weekIndex < 24 group by playerId order by sum(recYds) desc, sum(recTDs) desc LIMIT 10; select fullName, playerId, sum(defTotalTackles) "defTotalTackles", sum(defForcedFum) "defForcedFum", sum(defSacks) "defSacks" from defensive_stats where seasonIndex = 2 and weekIndex < 24 group by playerId order by sum(defTotalTackles) desc, sum(defForcedFum) desc LIMIT 10; select fullName, playerId, sum(defInts) "defInts", sum(defDeflections) "defDeflections", sum(defCatchAllowed) "defCatchAllowed", sum(defTDs) "defTDs" from defensive_stats where seasonIndex = 2 and weekIndex < 24 group by playerId order by sum(defInts) desc, sum(defDeflections) desc, sum(defCatchAllowed) asc LIMIT 10; select fullName, playerId, sum(defForcedFum) "defForcedFum", sum(defTotalTackles) "defTotalTackles", sum(defFumRec) "defFumRec", sum(defInts) "defInts" from defensive_stats where seasonIndex = 2 and weekIndex < 24 group by playerId order by sum(defForcedFum) desc, sum(defTotalTackles) desc LIMIT 10; select fullName, playerId, sum(fGMade) "fgMade", sum(fGAtt) "fgAtt", sum(xpMade) "xpMade" from kicking_stats where seasonIndex = 2 and weekIndex < 24 group by playerId order by sum(fGMade) desc, sum(fgAtt) asc LIMIT 10;'
let con = mysql.createConnection({ 
    "host": process.env.host,
    "user": process.env.user,
    "password": process.env.pw,
