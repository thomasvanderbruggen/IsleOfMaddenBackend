...
*/
let sql = 'select fullName, sum(passAtt) "passAtt", sum(passComp) "passComp", sum(passYds) "passYds", sum(passTDs) "passTDs" from passing_stats where seasonIndex = 2 group by playerId order by sum(passYds) desc, sum(passTDs) desc LIMIT 10; select fullName, sum(rushAtt) "rushAtt", sum(rushYds) "rushYds", sum(rushTDs) "rushTDs", max(rushLongest) "rushLongest" from rushing_stats group by playerId order by sum(rushYds) desc, sum(rushTDs) desc LIMIT 10; select fullName, sum(recCatches) "recCatches", sum(recYds) "recYds", sum(recTDs) "recTDs", max(recLongest) "recLongest" from receiving_stats where seasonIndex = 2 group by playerId order by sum(recYds) desc, sum(recTDs) desc LIMIT 10; select fullName, sum(defTotalTackles) "defTotalTackles", sum(defForcedFum) "defForcedFum", sum(defSacks) "defSacks" from defensive_stats where seasonIndex = 2 group by playerId order by sum(defTotalTackles) desc, sum(defForcedFum) desc LIMIT 10; select fullName, sum(defInts) "defInts", sum(defDeflections) "defDeflections", sum(defCatchAllowed) "defCatchAllowed", sum(defTDs) "defTDs" from defensive_stats where seasonIndex = 2 group by playerId order by sum(defInts) desc, sum(defDeflections) desc, sum(defCatchAllowed) asc LIMIT 10; select fullName, sum(defForcedFum) "defForcedFum", sum(defTotalTackles) "defTotalTackles", sum(defFumRec) "defFumRec", sum(defInts) "defInts" from defensive_stats where seasonIndex = 2 group by playerId order by sum(defForcedFum) desc, sum(defTotalTackles) desc LIMIT 10; select fullName, sum(fGMade) "fgMade", sum(fGAtt) "fgAtt", sum(xpMade) "xpMade" from kicking_stats where seasonIndex = 2 group by playerId order by sum(fGMade) desc, sum(fgAtt) asc LIMIT 10;'
let con = mysql.createConnection({ 
    "host": process.env.host,
    "user": process.env.user,
    "password": process.env.pw,
